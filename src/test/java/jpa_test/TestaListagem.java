...
public class TestaListagem {
	...
	public void shouldInsert() throws SQLException {
		try (Connection connection = Database.getConnection()) {
			try (Statement statement = connection.createStatement()) {
			System.out.println(statement.execute("insert into Produto(nome,descricao) "
					+ "values ('notebook','notebook semi-novo')	"));
	...
	public void shouldDelete() throws SQLException {
		try (Connection connection = Database.getConnection()) {
			try (Statement statement = connection.createStatement()) {
			System.out.println(statement.execute("delete from Produto "
					+ " where id > 3"));
